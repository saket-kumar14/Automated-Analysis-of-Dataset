...
def narrate_story(analysis, output_dir):
    # Generate a narrative using AI Proxy via POST request
    try:
        ...
        # Make the API request to the AI Proxy
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {AIPROXY_TOKEN}"
        }
        
        response = requests.post(API_PROXY_URL, headers=headers, data=json.dumps(payload))
        response_data = response.json()
        
        # Extract story from the response
        story = response_data.get('choices', [{}])[0].get('message', {}).get('content', "Error generating narrative.")
